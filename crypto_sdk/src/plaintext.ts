...
export function shuffleEncryptPlaintext(
  ...
  const B: bigint[] = [];
  for (let i = 0; i < 4; i++) {
    // console.log(A.length, X.slice(i * numCards, (i + 1) * numCards).length);
  ...
  // console.log(ECOutArr[0]);
  // console.log(ECOutArr.length); // 52
  // console.log(ECOutArr[0].length); // 2 c0, c1
  // console.log(ECOutArr[0][0].length); // 3 x, y
  const Y: bigint[] = [];
  for (let i = 0; i < 2; i++) {
    for (let j = 0; j < 2; j++) {
...
export function shuffleEncryptV2Plaintext(
  ...
  // recover delta from bit compressed deck
