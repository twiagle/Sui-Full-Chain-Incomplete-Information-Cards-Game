...
import { keyGen, bits2Num, ecCompress, ecDecompress, ecX2Delta, num2Bits, recoverDeck, sampleFieldElements, samplePermutation, prepareDecryptData } from "../src/utilities";
...
export class ZKShuffle {
  ...
  main = async () => {
    ...
    let shuffledEncrypedCompressedDeck = shuffleEncryptV2Plaintext(
      this.babyjub, Number(numCards), A, R, this.pk,
      deck.X0, deck.X1,
    ...
    // decrypt
    // console.log(typeof (X0[0]));
    // console.log(typeof (X1[0]));
    // console.log(typeof (selector0));
    // console.log(typeof (selector1));
    ...
    const x = decryptProof.publicSignals[0];
    const y = decryptProof.publicSignals[1];
    const poker = this.queryCardsPerX(x)
    console.log(poker);
